...
type AiAssistantProps = {
  className?: string
  /** Ref forwarded to the trigger button (landing intro morph). */
  triggerRef?: Ref<HTMLButtonElement>
  triggerClassName?: string
  triggerAriaLabel?: string
  triggerContent?: ReactNode
  /** Default FAB swaps to an X while open; landing keeps its mark. */
  swapTriggerWhenOpen?: boolean
}
...
export const AiAssistant = ({
  ...
  const openLabel = triggerAriaLabel ?? t("open")
  const triggerLabel =
    open && swapTriggerWhenOpen ? t("close") : openLabel
  const origin = locale === "ar" ? "bottom left" : "bottom right"
        ...
        {open ? (
          <motion.div
            ...
            initial={
              reducedMotion
                ? false
                : { opacity: 0, y: 18, scale: 0.88, filter: "blur(4px)" }
            }
            animate={{ opacity: 1, y: 0, scale: 1, filter: "blur(0px)" }}
            exit={
              reducedMotion
                ? undefined
                : { opacity: 0, y: 14, scale: 0.92, filter: "blur(3px)" }
            }
